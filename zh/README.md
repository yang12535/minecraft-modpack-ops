...
> **语言选项：** 本仓库默认提供中文版。英文版请打开仓库中的 [`en/` 目录](https://github.com/yang12535/minecraft-modpack-ops)。
...
具有技能目录约定的代理或框架应仅将 `zh/` 子目录复制或符号链接到该目录中。不要把整个仓库安装到会递归发现技能的目录中，因为根目录、`zh/` 和 `en/` 的入口使用相同的技能名称。其他情况下，可从代理的系统指令、项目指令或任务提示中直接引用根目录的 `SKILL.md`。
...
```text
准备归档此服务器。先确认无人在线，清理和限制备份，然后生成一份
已脱敏的玩家公开包，以及一份仅供运维者恢复使用的私有完整恢复包。
```
